...
public class SortedArraySet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  public static <T> SortedArraySet<T> copyOf(SortedSet<T> that) {
    Comparator<T> comparator = that.comparator();
    SortedArraySet<T> copyOf = new SortedArraySet<>(comparator);

    for (T element : that) {
      copyOf.append(element);  // Assuming append keeps the elements in sorted order
    }
  ...
  public void insert(T element) {
    ...
    //this process takes place if no element is found
    if(!finder.found){
      ensureCapacity();
      //we adapt the new capacity to insert the new element
      for (int i = size; i > finder.index; i--) {
        elements[i] = elements[i-1];
        // to insert f.e. '2'
        // [1, 1.5, 3] ---> [1, 1.5, 3, _] ---> [1, 1.5, 3, 3] ---> [1, 1.5, 2, 3]
      }
  ...
  public void delete(T element) {
    ...
    if(finder.found){
      ensureCapacity();
      //we adapt the new capacity to delete the new element
      for (int i = finder.index; i < size - 1; i++) {
        elements[i] = elements[i+1];
        // to delete f.e. '2'
        // [1, 1.5, 2, 3] ---> [1, 1.5, 3, 3] ---> [1, 1.5, 3, _]
      }
  ...
  public static <T> SortedArraySet<T> union(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    if (sortedSet1.comparator() != sortedSet2.comparator()) {
      throw new IllegalArgumentException("union: both sorted sets must use the same comparator");
    }
    Comparator<T> comparator = sortedSet1.comparator();
    SortedArraySet<T> unionSet = new SortedArraySet<>(comparator);
  ...
  public static <T> SortedArraySet<T> intersection(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    Comparator<T> comparator = sortedSet1.comparator();
    SortedArraySet<T> inter = new SortedArraySet<>(comparator);
    Iterator<T> it1 = sortedSet1.iterator();
    Iterator<T> it2 = sortedSet2.iterator();

    //it will check the nullity of the set
    T elem1 = nextOrNull(it1);
    T elem2 = nextOrNull(it2);
  ...
  public static <T> SortedArraySet<T> difference(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    Comparator<T> comparator = sortedSet1.comparator();
    SortedArraySet<T> diff = new SortedArraySet<>(comparator);
    ...
    while (elem1 != null) {
      if (elem2 == null) {
        // Add all remaining elements from sortedSet1, as sortedSet2 is exhausted
        diff.append(elem1);
        elem1 = nextOrNull(it1);
      } else {
        int cmp = comparator.compare(elem1, elem2);
        if (cmp < 0) {
          // elem1 is unique to sortedSet1
          diff.append(elem1);
          elem1 = nextOrNull(it1);
        } else if (cmp == 0) {
          // Both elements are equal, so skip elem1
          elem1 = nextOrNull(it1);
          elem2 = nextOrNull(it2);
        } else {
          // elem2 is smaller, move to the next element in sortedSet2
          elem2 = nextOrNull(it2);
        }
      }
    }
